...
namespace UniversalStorage2
{
  public class USdebugMessages : MonoBehaviour
  {
    ...
    public void
      debugMessage(object input) // fully automatic mode, posts to screen or or log depending on general setting
    {
      if (debugMode)
        switch (outputMode)
        {
    ...
    public void
      debugMessage(object input, bool postToLog,
        float postToScreenDuration) // fully manual mode, post to screen or log depending on parameters
    {
      if (debugMode) PostMessage(input, postToLog, postToScreenDuration);
    }

    public void
      debugMessage(object input,
        float postToScreenDuration) // semi-manual mode: posts to screen regardless of general setting, and to log, depending on general setting
    {
      if (debugMode)
        switch (outputMode)
        {
    ...
    public void
      PostMessage(object input, bool postToLog,
        float postToScreenDuration) // Posts uninstantiated, so it doesn't care about debugMode.
    {
      if (postToLog) Debug.Log(moduleName + input);
      if (postToScreenDuration > 0f) // will only work in the flight scene, gives an error in other places.
        ScreenMessages.PostScreenMessage(new ScreenMessage(input.ToString(), postToScreenDuration,
    ...
    public static void USStaticLog(string log, params object[] stringObjects)
    {
      ...
      var finalLog = string.Format("[Universal_Storage] {0}", log);
      Debug.Log(finalLog);
    }
  }
}