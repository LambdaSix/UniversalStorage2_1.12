...
namespace UniversalStorage2
{
  public class USAnimateGeneric : PartModule, IScalarModule
  {
    ...
    [KSPField] public string statusTitle = "Status";
    [KSPField] public string primaryAnimationName;
    [KSPField] public string secondaryAnimationName;
    [KSPField] public string primaryStartEventGUIName = "Deploy Primary Bays";
    [KSPField] public string primaryEndEventGUIName = "Retract Primary Bays";
    [KSPField] public string primaryToggleActionName = "Toggle Primary Bays";
    [KSPField] public string lockPrimaryDoorName = "Lock Primary Bays";
    [KSPField] public string unlockPrimaryDoorName = "Unlock Primary Bays";
    [KSPField] public string secondaryStartEventGUIName = "Deploy Secondary Bays";
    [KSPField] public string secondaryEndEventGUIName = "Retract Secondary Bays";
    [KSPField] public string secondaryToggleActionName = "Toggle Secondary Bays";
    [KSPField] public string lockSecondaryDoorName = "Lock Secondary Bays";
    [KSPField] public string unlockSecondaryDoorName = "Unlock Secondary Bays";
    [KSPField] public string combinedStartEventGUIName = "Deploy All Bays";
    [KSPField] public string combinedEndEventGUIName = "Retract All Bays";
    [KSPField] public string combinedToggleActionName = "Toggle All Bays";
    [KSPField(isPersistant = true)] public bool primaryDeployed = false;
    [KSPField(isPersistant = true)] public bool secondaryDeployed = false;
    [KSPField(isPersistant = true)] public bool combinedDeployed = false;
    ...
    [KSPField] public string primaryDeployLimitName = "Primary Bay Deploy Limit";
    [KSPField] public bool allowPrimaryDeployLimit = true;
    [KSPField] public bool primaryDeployLimitInFlight = false;

    [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true)]
    [UI_FloatRange(minValue = 0, maxValue = 100, stepIncrement = 1)]
    public float secondaryDeployLimit = 100;

    [KSPField] public string secondaryDeployLimitName = "Secondary Bay Deploy Limit";
    [KSPField] public bool allowSecondaryDeployLimit = true;
    [KSPField] public bool secondaryDeployLimitInFlight = false;
    [KSPField] public float customAnimationSpeed = 1f;
    [KSPField] public float scalarAnimationSpeed = 1f;
    [KSPField] public bool primaryAvailableInEVA = true;
    [KSPField] public bool primaryAvailableInVessel = true;
    [KSPField] public bool primaryAvailableInEditor = true;
    [KSPField] public bool primaryActionAvailable = true;
    [KSPField] public bool secondaryAvailableInEVA = false;
    [KSPField] public bool secondaryAvailableInVessel = false;
    [KSPField] public bool secondaryAvailableInEditor = false;
    [KSPField] public bool secondaryActionAvailable = false;
    [KSPField] public bool combinedAvailableInEVA = false;
    [KSPField] public bool combinedAvailableInVessel = false;
    [KSPField] public bool combinedAvailableInEditor = false;
    [KSPField] public bool combinedActionAvailable = false;
    [KSPField] public bool allowDoorLock = true;
    [KSPField(isPersistant = true)] public bool lockPrimaryDoors = false;
    [KSPField(isPersistant = true)] public bool lockSecondaryDoors = false;
    [KSPField] public bool jettisonAvailable = false;
    [KSPField] public bool jettisonDeployedOnly = false;
    [KSPField] public bool jettisonStowedOnly = false;
    [KSPField] public string jettisonIndices = string.Empty;
    [KSPField] public int dragModuleIndex = -1;
    [KSPField] public float EVArange = 5f;
    [KSPField] public bool oneShot = false;
    [KSPField] public string SwitchID = string.Empty;
    [KSPField] public string AnimationControlState = string.Empty;
    [KSPField] public bool ToggleDoorRadiators = false;
    [KSPField] public bool UseDoorObstructions = false;
    [KSPField] public string DoorObstructionTrigger = "DoorTrigger";
    [KSPField] public string PrimaryDoorObstructionSource = "RaySource";
    [KSPField] public string SecondaryDoorObstructionSource = "SecondaryRaySource";
    [KSPField] public string PrimaryDoorObstructionLength = string.Empty;
    [KSPField] public string SecondaryDoorObstructionLength = string.Empty;
    [KSPField] public bool ObstructionDebugLines = false;
    [KSPField] public bool SolarPanelToggle = false;

    [KSPField(isPersistant = true)] public int CurrentSelection = -1;
    ...
    public override void OnStartFinished(StartState state)
    {
      ...
      if (_animsPrimary != null && _animsPrimary.Length > 0)
      {
        ...
        for (var i = _animsPrimary.Length - 1; i >= 0; i--)
        {
          var anim = _animsPrimary[i];

          if (anim == null)
            continue;

          if (DebugMode)
            debug.debugMessage(string.Format("Primary Animation Clips: {0} - Name: {1}", anim.GetClipCount(),
              anim.clip.name));
      ...
      if (_animsSecondary != null && _animsSecondary.Length > 0)
      {
        ...
        lockEventSecondary.guiName =
          lockSecondaryDoors ? _localizedSecondaryUnlockString : _localizedSecondaryLockString;
        ...
        for (var i = _animsSecondary.Length - 1; i >= 0; i--)
        {
          var anim = _animsSecondary[i];

          if (anim == null)
            continue;

          if (DebugMode)
            debug.debugMessage(string.Format("Secondary Animation Clips: {0} - Name: {1}", anim.GetClipCount(),
              anim.clip.name));
      ...
      if (_animsPrimary != null && _animsPrimary.Length > 0 && _animsSecondary != null && _animsSecondary.Length > 0)
      {
        ...
        tglActionCombined.active =
          combinedActionAvailable && !(allowDoorLock && (lockSecondaryDoors || lockPrimaryDoors));
      ...
      primaryLimit.guiActiveEditor =
        allowPrimaryDeployLimit && !(allowDoorLock && lockPrimaryDoors) && _animsPrimary != null;
      primaryLimit.guiActive = allowSecondaryDeployLimit && !(allowDoorLock && lockPrimaryDoors) &&
                               primaryDeployLimitInFlight && _animsPrimary != null;

      secondaryLimit.guiActiveEditor = !(allowDoorLock && lockSecondaryDoors) && _animsSecondary != null;
      secondaryLimit.guiActive = !(allowDoorLock && lockSecondaryDoors) && secondaryDeployLimitInFlight &&
                                 _animsSecondary != null;

      if (CurrentSelection >= 0) UpdateEventStates();
      ...
      if (!string.IsNullOrEmpty(PrimaryDoorObstructionLength))
      {
        ...
        if (_PrimaryObstructionLengths.Length == 1 || _SwitchIndices == null || _SwitchIndices.Length == 0 ||
            CurrentSelection < 0)
      ...
      if (!string.IsNullOrEmpty(SecondaryDoorObstructionLength))
      {
        ...
        if (_SecondaryObstructionLengths.Length == 1 || _SwitchIndices == null || _SwitchIndices.Length == 0 ||
            CurrentSelection < 0)
    ...
    {
      var indices = USTools.parseIntegers(jettisonIndices).ToArray();
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
          for (var i = _animsPrimary.Length - 1; i >= 0; i--)
          {
            ...
            if (anim.gameObject.activeInHierarchy)
            {
              ...
              {
                if (DebugMode)
                  debug.debugMessage(string.Format("Forcing Animation Stop: {0} - Time: {1:F2}", primaryAnimationName,
                    anim[primaryAnimationName].normalizedTime));
              ...
              else
              {
                ...
                if (anim[primaryAnimationName].speed > 0)
                  if (_primaryAnimTime >= primaryDeployLimit * 0.01f)
                  {
                    ...
                    for (var j = _animsPrimary.Length - 1; j >= 0; j--)
                    {
                      if (j == i)
                        continue;

                      var otherAnim = _animsPrimary[j];
        ...
        if (_animsSecondary == null)
          ...
          for (var i = _animsSecondary.Length - 1; i >= 0; i--)
          {
            ...
            if (anim.gameObject.activeInHierarchy)
            {
              ...
              {
                if (DebugMode)
                  debug.debugMessage(string.Format("Forcing Animation Stop: {0} - Time: {1:F2}", secondaryAnimationName,
                    anim[secondaryAnimationName].normalizedTime));
              ...
              else
              {
                ...
                if (anim[secondaryAnimationName].speed > 0)
                  if (_secondaryAnimTime >= secondaryDeployLimit * 0.01f)
                  {
                    ...
                    for (var j = _animsSecondary.Length - 1; j >= 0; j--)
                    {
                      if (j == i)
                        continue;

                      var otherAnim = _animsSecondary[j];
      ...
      if (primaryAnimationState == ModuleAnimateGeneric.animationStates.CLAMPED ||
      ...
      else
      {
        ...
        else if (primaryAnimationState == ModuleAnimateGeneric.animationStates.LOCKED ||
                 secondaryAnimationState == ModuleAnimateGeneric.animationStates.LOCKED)
          status = _localizedLocked;
        else if (primaryAnimationState == ModuleAnimateGeneric.animationStates.FIXED ||
                 secondaryAnimationState == ModuleAnimateGeneric.animationStates.FIXED) status = _localizedFixed;
      }
    }
    ...
    private void OnPrimaryDeployChange(BaseField field, object obj)
    {
      ...
      var deploy = primaryDeployLimit * 0.01f;
    ...
    private void OnSecondaryDeployChange(BaseField field, object obj)
    {
      ...
      var deploy = secondaryDeployLimit * 0.01f;
    ...
    private void UpdateEventStates()
    {
      ...
      switch (_ControlStates[CurrentSelection])
      {
        ...
        case 1:
          tglEventPrimary.active = !(allowDoorLock && lockPrimaryDoors) &&
                                   (primaryAvailableInVessel || primaryAvailableInEVA || primaryAvailableInEditor);
        ...
        case 2:
          ...
          tglActionPrimary.active = false;
          tglEventSecondary.active = !(allowDoorLock && lockSecondaryDoors) &&
                                     (secondaryAvailableInVessel || secondaryAvailableInEVA ||
                                      secondaryAvailableInEditor);
        ...
        case 3:
          ...
          tglEventSecondary.active = !(allowDoorLock && lockSecondaryDoors) &&
                                     (secondaryAvailableInVessel || secondaryAvailableInEVA ||
                                      secondaryAvailableInEditor);
          tglActionSecondary.active = !(allowDoorLock && lockSecondaryDoors) && secondaryActionAvailable;
          tglEventCombined.active = !(allowDoorLock && (lockPrimaryDoors || lockSecondaryDoors)) &&
                                    (combinedAvailableInVessel || combinedAvailableInEVA || combinedAvailableInEditor);
          tglActionCombined.active =
            !(allowDoorLock && (lockPrimaryDoors || lockSecondaryDoors)) && combinedActionAvailable;
          lockEventPrimary.active = allowDoorLock;
          lockEventSecondary.active = allowDoorLock;
          break;
      }
    ...
    [KSPEvent(name = "jettisonEvent", guiName = "Jettison Doors", guiActive = false, guiActiveUnfocused = false,
      unfocusedRange = 5f, guiActiveEditor = false)]
    public void jettisonEvent()
    {
      for (var i = jettisonModules.Length - 1; i >= 0; i--) jettisonModules[i].OnJettison();

      jettEvent.active = false;
      jettAction.active = false;
    }

    [KSPEvent(name = "lockPrimaryDoorsEvent", guiName = "Lock Primary Bays", guiActive = false,
      guiActiveUnfocused = false, guiActiveEditor = true, active = false)]
    public void lockPrimaryDoorsEvent()
    {
      ...
      tglEventPrimary.active = !lockPrimaryDoors &&
                               (primaryAvailableInVessel || primaryAvailableInEVA || primaryAvailableInEditor);

      tglActionPrimary.active = !lockPrimaryDoors && primaryActionAvailable;

      tglEventCombined.active = !lockPrimaryDoors && !lockSecondaryDoors &&
                                (combinedAvailableInVessel || combinedAvailableInEVA || combinedAvailableInEditor);

      tglActionCombined.active = !lockPrimaryDoors && !lockSecondaryDoors && combinedActionAvailable;
    }

    [KSPEvent(name = "lockSecondaryDoorsEvent", guiName = "Lock Secondary Bays", guiActive = false,
      guiActiveUnfocused = false, guiActiveEditor = true, active = false)]
    public void lockSecondaryDoorsEvent()
    {
      ...
      tglEventSecondary.active = !lockSecondaryDoors &&
                                 (secondaryAvailableInVessel || secondaryAvailableInEVA || secondaryAvailableInEditor);
    ...
    [KSPEvent(name = "toggleEventCombined", guiName = "Deploy All Bays", guiActive = false, guiActiveUnfocused = false,
      unfocusedRange = 5f, guiActiveEditor = false)]
    ...
    [KSPEvent(name = "toggleEventPrimary", guiName = "Deploy Primary Bays", guiActive = false,
      guiActiveUnfocused = false, unfocusedRange = 5f, guiActiveEditor = false)]
    ...
    [KSPEvent(name = "toggleEventSecondary", guiName = "Deploy Secondary Bays", guiActive = false,
      guiActiveUnfocused = false, unfocusedRange = 5f, guiActiveEditor = false)]
    ...
    private bool PrimaryCollisionCheck()
    {
      ...
      for (var i = _PrimaryObstructionSources.Length - 1; i >= 0; i--)
      {
        var source = _PrimaryObstructionSources[i];

        var r = new Ray(source.position, source.forward);
        ...
        if (Physics.Raycast(r, out hit, _primaryObstructionLength, 1 << 0))
          if (hit.collider != null)
            ...
            {
              var p = Part.FromGO(hit.transform.gameObject);

              if (p != null)
              {
                ScreenMessages.PostScreenMessage(
                  Localizer.Format("#autoLOC_US_PrimaryBayObstruction", p.partInfo.title, part.partInfo.title), 10f,
                  ScreenMessageStyle.UPPER_CENTER);

                if (DebugMode)
                  debug.debugMessage(string.Format("Primary door obstruction detected; stopping animation: {0}",
                    p.partName));
    ...
    private bool SecondaryCollisionCheck()
    {
      ...
      for (var i = _SecondaryObstructionSources.Length - 1; i >= 0; i--)
      {
        var source = _SecondaryObstructionSources[i];
        ...
        if (Physics.Raycast(r, out hit, _secondaryObstructionLength, 1 << 0))
          if (hit.collider != null)
            if (hit.collider.attachedRigidbody != null)
            {
              ...
              {
                ScreenMessages.PostScreenMessage(
                  Localizer.Format("#autoLOC_US_SecondaryBayObstruction", p.partInfo.title, part.partInfo.title), 10f,
                  ScreenMessageStyle.UPPER_CENTER);

                if (DebugMode)
                  debug.debugMessage(string.Format("Secondary door obstruction detected; stopping animation: {0}",
                    p.partName));
    ...
    public bool CanMove =>
      _animsPrimary != null && _animsPrimary.Length > 0 ||
    ...
    public string ScalarModuleID => _id;
    ...
    public bool IsMoving()
    {
      ...
      return primaryAnimationState == ModuleAnimateGeneric.animationStates.MOVING ||
             secondaryAnimationState == ModuleAnimateGeneric.animationStates.MOVING;
    }
    ...
    public void SetScalar(float t)
    {
      ...
      if (_animsPrimary == null || _animsPrimary.Length <= 0) primary = false;

      if (_animsSecondary == null || _animsSecondary.Length <= 0) secondary = false;

      if (oneShot && primaryAnimationState == ModuleAnimateGeneric.animationStates.FIXED) primary = false;

      if (oneShot && secondaryAnimationState == ModuleAnimateGeneric.animationStates.FIXED) secondary = false;
      ...
      if (primary && secondary)
        ...
        time = _primaryAnimTime;
      else if (secondary) time = _secondaryAnimTime;
