...
namespace UniversalStorage2
{
  public class USSwitchControl : PartModule
  {
    ...
    [KSPField] public int SwitchID = -1;
    [KSPField] public string ObjectNames = string.Empty;
    [KSPField] public string VariantColors = string.Empty;
    [KSPField] public bool ShowInfo = true;
    [KSPField] public bool UpdateSymmetry = true;
    [KSPField] public bool FuelSwitchModeOne = false;
    [KSPField] public bool FuelSwitchModeTwo = false;
    [KSPField] public bool DebugDragCube = false;
    [KSPField(isPersistant = true)] public int CurrentSelection = 0;
    [KSPField] public string CurrentVariantTitle = "Current Variant";
    [KSPField] public string ModuleDisplayName = "Switch Control";

    [KSPField]
    [UI_USVariantSelector(affectSymCounterparts = UI_Scene.Editor, controlEnabled = true, scene = UI_Scene.Editor)]
    ...
    public override void OnLoad(ConfigNode node)
    {
      ...
      if (!HighLogic.LoadedSceneIsFlight && !HighLogic.LoadedSceneIsEditor)
        ...
        onUSEditorIconSwitch =
          GameEvents.FindEvent<EventData<int, int, AvailablePart, Transform>>("onUSEditorIconSwitch");
    }
    ...
    public override void OnStart(StartState state)
    {
      ...
      if (SwitchID < 0)
        for (var i = part.Modules.Count - 1; i >= 0; i--)
      ...
      if (state == StartState.Editor)
      {
        ...
        for (var i = 0; i < SwitchNames.Length; i++)
        {
          ...
          var primary = "#3a562a";
          var secondary = "#999999";

          if (SwitchColors != null && i < SwitchColors.Length)
          {
            var colors = SwitchColors[i].Split(',');
          ...
          _variantInfoList.Add(new USVariantInfo(_localizedCurrentVariantString, _localizedSwitchNames[i], primary,
            secondary));
        }

        var variantSelector = Fields["VariantSelection"].uiControlEditor as UI_USVariantSelector;
    ...
    public override string GetInfo()
    {
      ...
      {
        var variantList = USTools.parseNames(ObjectNames, false, true, string.Empty).ToArray();

        var info = StringBuilderCache.Acquire();
        info.AppendLine(Localizer.Format("#autoLOC_US_PartVariants"));

        for (var i = 0; i < variantList.Length; i++) info.AppendLine(Localizer.Format(variantList[i]));
    ...
    private void SwitchTo(bool player)
    {
      ...
      if (UpdateSymmetry && player)
        ...
        {
          var symSwitch = part.symmetryCounterparts[i].GetComponents<USSwitchControl>();
    ...
    [KSPEvent(name = "RenderDragCube", guiName = "Render Drag Cube", active = false, guiActive = true,
      guiActiveEditor = true)]
    public void RenderDragCube()
    {
      var cube = DragCubeSystem.Instance.RenderProceduralDragCube(part);
    ...
    {
      var cubeStrings = new List<string>();

      cubeStrings.Add(CurrentSelection.ToString());

      for (var i = 0; i < 6; i++) cubeStrings.Add(string.Format("{0:F4},{1:F4},{2:F4}", area[i], drag[i], depth[i]));
