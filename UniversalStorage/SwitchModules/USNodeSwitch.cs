...
namespace UniversalStorage2
{
  ...
  {
    [KSPField] public string AttachNodes = string.Empty;
    [KSPField] public bool ShiftNodes = false;
    [KSPField] public string ShiftedNodeNames = string.Empty;
    ...
    private void UpdateAttachNodes()
    {
      ...
      for (var j = _Nodes[i].Count - 1; j >= 0; j--)
        _Nodes[i][j].SetNodeState(false);

      if (DebugMode)
        debug.debugMessage(string.Format("Activating Node Group: {0}", CurrentSelection));

      for (var i = 0; i < _Nodes[CurrentSelection].Count; i++) _Nodes[CurrentSelection][i].SetNodeState(true);
    }
    ...
    private void UpdateShiftNodes(int old)
    {
      ...
      if (DebugMode)
        debug.debugMessage(string.Format("Updating Attach Position - Old: {0} - New: {1}", _ShiftedNodes[old].id,
          _ShiftedNodes[CurrentSelection].id));

      var previousNode = _ShiftedNodes[old];

      var newNode = _ShiftedNodes[CurrentSelection];
        ...
        debug.debugMessage(
          string.Format(
            ...
            , previousNode.attachedPart == null
              ? "Null"
              : (previousNode.attachedPart == EditorLogic.RootPart).ToString()
