...
namespace UniversalStorage2
{
  ...
  {
    [KSPField] public string JettisonTransforms = string.Empty;
    [KSPField] public int JettisonModuleIndex = -1;
    [KSPField] public bool ShowJettisonUI = false;
    [KSPField(isPersistant = true)] public bool Jettisoned;
    ...
    {
      for (var i = _JettisonTransforms.Length - 1; i >= 0; i--) _JettisonTransforms[i].gameObject.SetActive(false);
    }
    ...
    private IEnumerator SetJettisonTransform()
    {
      ...
      for (var i = _JettisonTransforms.Length - 1; i >= 0; i--)
      {
        var t = _JettisonTransforms[i];
