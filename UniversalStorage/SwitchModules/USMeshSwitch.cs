...
namespace UniversalStorage2
{
  public class USMeshSwitch : PartModule
  {
    ...
    [KSPField] public string MeshTransforms = string.Empty;
    ...
    [KSPField] public bool DeleteUnused = true;
    ...
    public override void OnStart(StartState state)
    {
      ...
      for (var i = _Transforms.Count - 1; i >= 0; i--)
      {
        if (i == CurrentSelection)
          continue;

        for (var j = _Transforms[i].Count - 1; j >= 0; j--)
        {
          if (DebugMode)
    ...
    private void LoadMeshNameData()
    {
      ...
      for (var i = 0; i < _Transforms.Count; i++)
      {
        ...
        for (var j = 0; j < _Transforms[i].Count; j++)
        {
          if (DebugMode)
    ...
    private void UpdateEditorMesh(int selection, Transform icon)
    {
      ...
      for (var i = _TransformNames.Count - 1; i >= 0; i--)
      for (var j = _TransformNames[i].Count - 1; j >= 0; j--)
      ...
      for (var i = 0; i < _TransformNames[selection].Count; i++)
    ...
    {
      if (DebugMode)
        debug.debugMessage(string.Format("Updating Mesh - Selection: {0} - Count: {1}", CurrentSelection,
          _Transforms.Count));
      ...
      for (var j = _Transforms[i].Count - 1; j >= 0; j--)
      {
        ...
        if (AffectColliders)
        {
          var coll = _Transforms[i][j].gameObject.GetComponent<Collider>();
      ...
      for (var i = 0; i < _Transforms[CurrentSelection].Count; i++)
      {
        if (DebugMode)
        ...
        if (AffectColliders)
        {
          var coll = _Transforms[CurrentSelection][i].gameObject.GetComponent<Collider>();
