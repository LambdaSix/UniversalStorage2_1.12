...
namespace UniversalStorage2.StockVariants
{
  ...
  public class UI_USPartActionVariantSelector : UIPartActionVariantSelector
  {
    ...
    public override void Setup(UIPartActionWindow window, Part part, PartModule partModule, UI_Scene scene,
      UI_Control control, BaseField field)
    {
    ...
    {
      for (var i = 0; i < _variantSelector.Variants.Count; i++)
      {
        var info = _variantSelector.Variants[i];
      ...
      var addedWidth = _variantLabel.preferredWidth - 40 + 4;

      if (addedWidth < 0)
        addedWidth = 0;

      _variantLabel.rectTransform.sizeDelta = new Vector2(_variantLabel.rectTransform.sizeDelta.x + addedWidth,
        _variantLabel.rectTransform.sizeDelta.y);
      _variantLabel.rectTransform.anchoredPosition = new Vector2(
        _variantLabel.rectTransform.anchoredPosition.x + addedWidth / 2,
        _variantLabel.rectTransform.anchoredPosition.y);

      variantName.rectTransform.sizeDelta = new Vector2(variantName.rectTransform.sizeDelta.x - addedWidth,
        variantName.rectTransform.sizeDelta.y);
      variantName.rectTransform.anchoredPosition = new Vector2(
        variantName.rectTransform.anchoredPosition.x + addedWidth / 2, variantName.rectTransform.anchoredPosition.y);
    ...
    {
      var button = Instantiate(prefabVariantButton, scrollMain.content);

      var stockButton = button.GetComponent<UIPartActionVariantButton>();

      var invalid = stockButton.imageInvalid;
      var primary = stockButton.imagePrimaryColor;
      var secondary = stockButton.imageSecomdaryColor;
      var selected = stockButton.imageSelected;
      var main = stockButton.buttonMain;

      DestroyImmediate(stockButton);

      var usButton = button.AddComponent<UI_USPartActionVariantButton>();
