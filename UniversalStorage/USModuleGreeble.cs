...
namespace UniversalStorage2
{
  ...
  {
    [KSPField] public string BottomNodeName = "";
    [KSPField] public bool CheckBottomNode = true;
    [KSPField] public bool AllowGreebleToggle = true;
    [KSPField] public string GreebleToggleName = "Toggle Details";
    [KSPField] public string GreebleTransform = "";
    [KSPField(isPersistant = true)] public bool IsActive = true;
    ...
    public override void OnStart(StartState state)
    {
      ...
      {
        var nodes = BottomNodeName.Split(';');

        for (var i = nodes.Length - 1; i >= 0; i--)
        {
          var node = part.FindAttachNode(nodes[i]);
      ...
      if (IsActive)
      {
        if (CheckBottomNode && bottomNodes != null && bottomNodes.Count > 0)
          for (var i = bottomNodes.Count - 1; i >= 0; i--)
          {
            var node = bottomNodes[i];
            ...
            if (node.attachedPart == null)
              for (var j = greebles.Count - 1; j >= 0; j--)
          ...
          for (var i = greebles.Count - 1; i >= 0; i--)
    ...
    [KSPEvent(name = "ToggleGreeble", guiName = "Toggle Details", guiActive = false, guiActiveUnfocused = false,
      guiActiveEditor = true, active = true)]
